...
def main(args):
    ...
    output_path = '/home/md.hassan/charts/ChartIE/results_det1/'
    ...
    for image_name in tq.tqdm(os.listdir(image_path)[:50]):
        ...
        # matched_boxes1 = plot_matched_boxes(samples_, giou_matrix_legend_text, ocr_boxes, legend_text_bboxes)

        # # legend_text_bboxes = np.array(legend_text_bboxes).astype(np.int32)
        # # matched_boxes2 = plot_matched_boxes(samples_, giou_matrix_legend_ele, ocr_boxes, legend_text_bboxes)

        # tick_bboxes = np.array(tick_bboxes).astype(np.int32)
        # try:
        #     matched_tick_text_boxes = plot_matched_boxes(samples_, giou_matrix_tick, ocr_boxes, tick_bboxes)
        # except:
        #     continue        

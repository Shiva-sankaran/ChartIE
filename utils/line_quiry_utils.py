...
def check_cross(keys, hybrids):
    ...
    for hybrid in hybrids:
        # create 8x8 box around each 'hybrid' point and see if 'key' lies inside
...
def GroupQuiry_no_ocr(image, keys_raw, hybrids_raw):
    keys = []
    for temp in keys_raw.values():
        for point in temp:
            bbox = [point[3], point[4], 6, 6]
            ...
            keys.append({'bbox': bbox, 'category_id': category_id,
                        'score': score, 'tag': tag})
    hybrids = []
    for temp in hybrids_raw.values():
        for point in temp:
    ...
    # for key in keys:
    #     if key['score'] > 0.4:
    #         drawLine(image, key['bbox'][0], key['bbox'][1], 3,
    #                  3, (int(255 * key['score']), 0, 0), key['tag'])
    #         y = plot_area[3] - key['bbox'][1]
    #         frac_y = (y) / (plot_area[3] - plot_area[1])
    #         drawData(image, key['bbox'][0], key['bbox'][1],
    #                  3, 3, frac_y*(max_value-min_value)+min_value)
    # for key in hybrids:
    #     if key['score'] > 0.4:
    #         drawLine(image, key['bbox'][0], key['bbox'][1], 7,
    #                  7, (0, int(255 * key['score']), 0), key['tag'])
    keys = get_point(keys, 0.4)
    hybrids = get_point(hybrids, 0.4)
    keys = check_cross(keys, hybrids)
    quiries = quiry_for_hybrid(keys)

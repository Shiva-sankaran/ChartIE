...
def GroupLine(image, keys, hybrids, plot_area, pair_info,  flag, min_value = None,  max_value = None):
        ...
        if len(line) > 1:
            ...
            for point in line:
                if point is not None:
                    data_line.append([point['bbox'][0], point['bbox'][1], point['tag']])
            if flag == True:
                data_line = get_data(data_line, plot_area)
            data_points.append(data_line)
    return data_points, union_result

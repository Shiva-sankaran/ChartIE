...
def random_crop(image, detections, random_scales, new_size, padding): #same function as OG
...
# below from ChartOCR
def random_crop_cls(image, detections, offset_image, random_scales, view_size, border=64):
    view_height, view_width   = (view_size['h'], view_size['w'])
    image_height, image_width = image.shape[0:2]

    scale  = np.random.choice(random_scales)
    offset_image /= scale                     # EXTRA from vanilla random_crop
    ...
    # one EXTRA return value from vanilla random_crop
